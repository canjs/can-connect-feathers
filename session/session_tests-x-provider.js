...
module.exports = function runSessionTests (options) {
	...
	QUnit.test('authenticate type=token', function (assert) {
		...
		user.save().then(function (createdUser) {
			...
			.then(function (newSession) {
				...
				app.authentication.getAccessToken().then(function (accessToken) {
					return app.logout().then(function() { return accessToken; });
	...
	QUnit.test('Session.current populates on login, clears on logout', function (assert) {
		...
		new User({
			...
			if(Session.current) {
				return done(new Error("Session.current should not exist yet."));
			}
	...
	QUnit.test('Session.current states', function(assert){
		...
		assert.equal(Session.current, null, 'Session.current should be null or undefined');

		var handler = function(ev, value){
			assert.equal(value, null, 'Session.current should be null for non-authenticated');
		};

...
* `tokenLocation` - this is the key name of the location in the `storage` engine.  The default is `ssr-cookie`, which is the same default cookie name used by the [feathers-done-ssr](https://www.npmjs.com/package/feathers-done-ssr) package. The values will need to match on both packages in order for the SSR server to send authenticated requests on behalf of the user.
...
## Changelog
- `0.3.0` - The `rest` methods now use a Promise instead of a Deferred.
